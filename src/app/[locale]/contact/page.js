...
export async function generateMetadata({ params }) {
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}/contact/${encodeURIComponent(
        locale
      )}`,
      {
        cache: "force-cache",
...
export default async function Contact({ params }) {
  ...
  try {
    ...
    console.error(
      `[2025-05-29T14:37:00.000Z] Failed to fetch contact data: ${error.message}`
    );
  }

  console.log("page data fetched:", contactData);

  return (
    <>
      <InnerBanner
        title={contactData?.banner_section?.title}
        mobile_image={contactData?.banner_section?.mobile_banner}
        banner_image={contactData?.banner_section?.web_banner}
        banner_alt={
          contactData?.banner_section?.web_banner_alt || "Banner Image"
        }
      ...
      />
      <FaqSection
        faqData={contactData?.faq_section}
        socialData={contactData?.social_link_section}
      />
    </>
  );
}
