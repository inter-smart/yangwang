  ...
  return (
    <div className="flex flex-wrap w-full relative z-1 overflow-hidden">
      <div className="w-full md:w-[300px] xl:w-[350px] 2xl:w-[500px] 3xl:w-[585px] md:pr-[25px] 2xl:pr-[40px] max-md:mb-[20px] text-white">
        <div className="realtive overflow-y-auto w-full max-h-[376px] sm:max-h-[468px] md:max-h-[500px] lg:max-h-[500px] xl:max-h-[500px] 2xl:max-h-[640px] 3xl:max-h-[668px] text-left [&::-webkit-scrollbar]:[width:8px] [&::-webkit-scrollbar-thumb]:bg-base1 [&::-webkit-scrollbar-track]:bg-base1/20">
          {data.map((branch, index) => (
            <div
              key={index}
              className={`3xl:px-[35px] 2xl:px-[30px] lg:px-[20px] px-[10px] 3xl:py-[30px] 2xl:py-[25px] lg:py-[20px] py-[10px] relative cursor-pointer transition group ${
                selectedBranch.id === branch.id
                  ? "bg-[#5949A7] active"
                  : "bg-[#5949A7]/10 hover:bg-[#5949a7bd]"
              }`}
              onClick={() => handleBranchClick(branch)}
            >
              <div
                className={cn(
                  "absolute z-1 3xl:top-[30px] 2xl:top-[20px] sm:top-[10px] top-[5px] 3xl:right-[35px] 2xl:right-[25px] sm:right-[10px] right-[5px] flex items-center justify-end transition-opacity duration-300 group-hover:opacity-100 group-hover:visible",
                  selectedBranch.id === branch.id
                    ? "opacity-100 visible"
                    : "opacity-0 invisible"
                )}
              >
                <div className="px-[2px]">
                  <a
                    ...
                    target="_blank"
                    className="3xl:w-[30px] sm:w-[25px] w-[20px] aspect-square block transition-transform hover:scale-105"
                  >
                    <Image
                      src="/images/icon-phone-1.svg"
                      alt="phone"
                      width={30}
                      height={30}
                      className="block"
              ...
              <div className="w-full max-w-[380px] flex items-start">
                <div className="w-[15px] sm:w-[20px] xl:w-[25px] aspect-square flex">
                  {selectedBranch.id === branch?.id ? (
                    <Image
                      src="/images/icon-showroom-2.svg"
                      alt="showroom2"
                      width={20}
                      height={26}
                      className="block"
                    ...
                    <Image
                      src="/images/icon-showroom-1.svg"
                      alt="showroom1"
                      width={20}
                      height={26}
                      className="block"
                ...
                <div className="xl:w-[calc(100% - 30px)] w-[calc(100% - 25px)] px-[10px]">
                  <Heading
                    size="heading6"
                    as="h6"
                    animation={false}
                    className="capitalize text-white mb-2 sm:mb-4"
                  >
                    {branch?.title}
                  </Heading>
                  <Text
                    ...
                    as="div"
                    animation={false}
                    className="max-w-full text-white mb-0.5 sm:mb-1"
                  >
                    {parse(branch?.address)}
                  </Text>
                  <Text
      ...
      <div className="w-full md:w-[calc(100%-300px)] xl:w-[calc(100%-350px)] 2xl:w-[calc(100%-500px)] 3xl:w-[calc(100%-585px)] max-md:h-[420px] relative z-0">
        <MapContainer
          center={mapCenter}
          zoom={12}
          className="h-full w-full z-1 max-h-[468px] md:max-h-[500px] lg:max-h-[500px] xl:max-h-[500px] 2xl:max-h-[640px] 3xl:max-h-[668px] overflow-hidden"
