...
export function PlaceholdersAndVanishInput({ placeholders, onChange, onSubmit }) {
  ...
  const subscribeToNewsletter = async (email) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch("https://www.yangwang.dev20.intersmarthosting.in/api/news-letter-subscription", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        throw new Error(result.errors?.email?.[0] || "Failed to subscribe to newsletter");
      }

      setSuccess(result.message || "Successfully subscribed to the newsletter!");
      setError(null);
    } catch (err) {
  ...
  return (
    <div className="relative w-full max-w-xl mx-auto">
      ...
      >
        ...
        />
        <button
          disabled={!value || isLoading || animating}
          type="submit"
      ...
      {error && (
        <div id="error-message" className="mt-2 text-red-500 text-sm text-center" role="alert">
          {error}
        </div>
      )}
      {success && (
        <div id="success-message" className="mt-2 text-green-500 text-sm text-center" role="alert">
          {success}
        </div>
      )}
    </div>
...
//       {success && <div className="mt-2 text-green-500 text-sm text-center">Successfully subscribed to the newsletter!</div>}
//     </div>
//   );
// }
