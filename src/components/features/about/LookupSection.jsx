...
const lookupInfo = [
  ...
  },
  {
    ...
    points: ["Symbol of vital life force", "Can be subtly used in the sky like energy waves or light ripples"],
  },
];
export default function LookupSection({ data }) {
  return (
                  ...
                  <div className="w-full h-full block max-md:pl-[15px]">
                    ...
                    <ul className="text-left text-white list-disc pl-5 mt-[20px] xl:mt-[30px] space-y-2">
                      {item.description?.split(",").map((point, i) => (
                        <li
                          key={i}
                          className="3xl:text-[18px] 2xl:text-[14px] md:text-[12px] text-[12px] font-normal leading-normal"
                        >
                          {point}
                        </li>
                      ))}
