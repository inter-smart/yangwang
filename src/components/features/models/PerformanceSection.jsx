    ...
    <section className="w-full h-auto lg:h-dvh lg:min-h-[368px] xl:min-h-[460px] 3xl:min-h-[768px] block py-[40px] lg:py-[60px] xl:py-[80px] 2xl:py-[100px] 3xl:py-[120px] bg-black overflow-hidden relative z-0 before:content-[''] before:block before:absolute before:-z-1 before:inset-0 before:w-full before:h-full before:bg-black/20 before:pointer-events-none">
      <AnimatePresence mode="wait">
        ...
        >
          {activeItem?.media?.type === "video" ? (
            ...
            <picture>
              <source media="(max-width: 768px)" srcSet={activeItem?.media?.mobile_banner?.url} />
              <Image
      ...
      <div className="container h-full">
        <div className="h-full flex flex-col justify-between">
          <Heading size="heading3" as="h3" className="text-center capitalize text-white mb-[60px] xl:mb-[40px] 3xl:mb-[60px]">
            {activeItem?.title}
          </Heading>
          <Accordion type="single" collapsible className="w-full" defaultValue={`item-${activeIndex}`}>
            {performanceData?.map((item, index) => (
              <AccordionItem key={`accordion-${index}`} value={`item-${index}`} className="border-none">
                <AccordionTrigger
